...
#define OP_HTMLSTAT 10
  ...
  if (issyso() && skipsp(&clicnt, &clipoi))
  {
    ...
    switch (uCmd)
    {
      ...
      case OP_ADD :
#ifndef AXIPR_UDP
        route_add((unsigned char*)&hostname, host, call, (uMode == UDP_MODE ? port : 0), (call[0] == 0 ? TRUE : FALSE), 0);
        break;
#else
...
void dump_ax25ip(MBHEAD* mbp)
{
  ...
  for (i = 0; i < route_tbl_top; ++i)
  {
    /* Dynamisch gelernte Routen werden nicht geschrieben ! */
    if (route_tbl[i].timeout != 0)
      continue;
...
int setup_ip(void)
{
  ...
  if (sockopt_keepalive == TRUE)
  {
    ...
    if (setsockopt(tmp_fd, SOL_SOCKET, SO_KEEPALIVE, &iFlag, sizeof(int)) < 0)
    {
      xprintf("AX25IP : cannot set SO_KEEPALIVE for ip : %s\n", strerror(errno));
      LOGL2("AX25IP : cannot set SO_KEEPALIVE for ip : %s\n", strerror(errno));
    }
  }
  ...
  if (sockopt_throughput == TRUE)
  {
    ...
    if (setsockopt(tmp_fd, IPPROTO_IP, IP_TOS, &iFlag, sizeof(IPTOS_THROUGHPUT)) < 0)
    {
      xprintf("AX25IP : cannot set IPTOS_THROUGHPUT for ip : %s\n", strerror(errno));
      LOGL2("AX25IP : cannot set IPTOS_THROUGHPUT for ip : %s\n", strerror(errno));
    }
  }
